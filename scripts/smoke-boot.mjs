...
//
// Exit 0 = mounted, boots clean, and removal restores the stock tree. Temp
// dir is kept and printed on failure, removed on success.
//
// dsh-ask-router note: with no UI surfaces registered the waterfall answerer
// simply delegates (next()); the smoke proves the plugin LOADS and APPLIES
// under the real host, which is exactly the gate (a broken import/inject/
// compose dies with a loader error).
...
const ownName = pkg.name // @aiwayds/dsh-ask-router

const work = mkdtempSync(path.join(tmpdir(), 'dsh-ask-router-smoke-'))
...
if (dumpAfter.stdout.includes('dsh-ask-router')) {
